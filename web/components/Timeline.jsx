...
export default function Timeline({ timeline, characters, protagonist, side, note }) {
  const rows = useMemo(() => {
      ...
      {rows.map((r) => (
        <div key={r.name} style={{ display: "flex", alignItems: "center", margin: "3px 0" }}>
          <div style={{ width: 80, textAlign: "right", paddingRight: 6, fontSize: 12,
            color: r.name === side ? "#e57ba0" : r.name === protagonist ? "#e6b866" : "#c8d0dc",
            fontWeight: r.name === side || r.name === protagonist ? 700 : 400 }}>
            {r.name}
          </div>
